...
fn main() {
	...
	let mut profile_data = Profile {status_text: None, status_emoji: None, status_expiration: 0};
	...
	let token = matches.value_of("token").unwrap_or("--token=TOKEN is required");

	let profile_request = ProfileRequest { profile: profile_data };
